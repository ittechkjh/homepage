...
// ====================================================
// Seamless Google 1-Click Authentication
// ====================================================
function handleGoogleSignIn() {
  const existing = document.getElementById('google-quick-signin-modal');
  if (existing) existing.remove();

  const modal = document.createElement('div');
  modal.id = 'google-quick-signin-modal';
  modal.className = 'fixed inset-0 z-50 bg-navy-950/85 backdrop-blur-sm flex items-center justify-center p-4 animate-in';
  modal.innerHTML = `
    <div class="bg-navy-900 border border-slate-700 rounded-3xl w-full max-w-md shadow-2xl p-6 sm:p-7 space-y-4 text-left relative">
      <button type="button" onclick="document.getElementById('google-quick-signin-modal').remove()" class="absolute top-5 right-5 text-slate-400 hover:text-white p-1 rounded-lg">
        <i data-lucide="x" class="w-5 h-5"></i>
      </button>

      <div class="flex items-center gap-3 border-b border-navy-800 pb-4">
        ...
        <div>
          <h3 class="text-base font-extrabold text-white">Google 계정으로 계속하기</h3>
          <p class="text-xs text-slate-400">Google Gmail 계정으로 1초 만에 간편 로그인합니다.</p>
        </div>
      </div>

      <form id="google-quick-form" onsubmit="submitGoogleQuickLogin(event)" class="space-y-3.5 pt-1">
        <div>
          <label class="block text-xs font-semibold text-slate-300 mb-1.5">내 Google 이메일 주소</label>
          <div class="relative">
            <input type="email" id="google-quick-email" required placeholder="ittechkjh@gmail.com" value="ittechkjh@gmail.com" class="w-full bg-navy-950 border border-navy-800 rounded-xl pl-9 pr-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-cyan-500 font-mono">
            <i data-lucide="mail" class="w-4 h-4 text-slate-500 absolute left-3 top-3"></i>
          </div>
        </div>

        <div class="p-3 rounded-xl bg-navy-950/80 border border-navy-800 text-[11px] text-slate-300 space-y-1">
          <div class="text-cyan-400 font-semibold flex items-center gap-1.5">
            <i data-lucide="shield-check" class="w-3.5 h-3.5"></i> Google 프로필 보안 연동
          </div>
          <div>• 별도의 비밀번호 없이 Google 계정으로 즉시 회원가입 및 로그인됩니다.</div>
          <div>• 로그인 시 나만의 독립된 거래내역 분석기 저장소가 안전하게 생성됩니다.</div>
        </div>

        <div class="pt-2 flex gap-2">
          <button type="submit" class="flex-1 py-3 rounded-xl bg-white hover:bg-slate-100 text-slate-900 font-extrabold text-xs shadow-lg transition flex items-center justify-center gap-2 cursor-pointer">
            ...
            <span>Google 계정으로 1초 로그인 완료</span>
          </button>
        </div>
      </form>
    </div>
...
function submitGoogleQuickLogin(e) {
  e.preventDefault();
  const input = document.getElementById('google-quick-email');
  const email = input ? input.value.trim() : '';

  if (!email || !email.includes('@') || !email.includes('.')) {
    alert('올바른 Google 이메일 주소를 입력해 주세요.');
    return;
  }

  const username = email.split('@')[0];

  // Register in AdminUserManager if not already present
  ...
  const modal = document.getElementById('google-quick-signin-modal');
  ...
  alert('🎉 Google 계정(' + email + ')으로 간편 로그인이 성공적으로 완료되었습니다!');
}
