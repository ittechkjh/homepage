...
const App = {
    ...
    saveTrades: function () {
        try {
            localStorage.setItem('upbit_saved_trades', JSON.stringify(this.state.rawTrades));
        } catch (err) {
            console.warn('로컬 저장소 용량 초과:', err);
        }
    },

    loadSavedTrades: function () {
        try {
            const saved = localStorage.getItem('upbit_saved_trades');
            if (saved) {
                const parsed = JSON.parse(saved);
                if (Array.isArray(parsed) && parsed.length > 0) {
                    this.state.rawTrades = parsed;
                    this.recalculate();
                    this.fetchLiveTickers(false);
                }
            }
    ...
    clearData: function () {
        ...
        localStorage.removeItem('upbit_saved_trades');
        localStorage.removeItem('upbit_custom_staking');
        this.recalculate();
        this.showToast('모든 거래 및 스테이킹 데이터가 초기화되었습니다.', 'info');
    },
